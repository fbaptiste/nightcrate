...
**Status:** `[shipped]` (v0.41.0 gave the unresolved queue an Admin API/UI; the resolver itself stays a pure service)
...
- **Route / API:** the service is consumed by the ingest pipeline (v0.40.0+) and the v0.41.0 attribution pass; its unresolved queue is exposed at `/api/equipment/unresolved` (Admin UI, v0.41.0)
...
- **Known limitation (resolved one layer up):** two physically distinct same-model cameras share one alias → the resolver returns whichever camera the alias points at; v0.41.0's `services/rig_attribution.py` corrects it to the attributed rig's actual body. Also as of v0.41.0, canonical line names (`Ha`/`Oiii`/…) no longer record unresolved observations — a global alias would be wrong for dual-rig users, so rig scoping is the only correct resolution
...
### Equipment Resolution + Rig Attribution (v0.41.0)

**Status:** `[shipped]`

Makes the cataloged data *right* — which rig, which camera, which physical filter — before v0.41.1 builds a correction UI on top of it. Three parts. (1) **Rig attribution** (`services/rig_attribution.py`, pure service): candidates are exactly the rigs assigned to the project via `project_rig` (no rigs assigned → no discovery, by design), and each available signal only *eliminates* a candidate — camera identity, `FOCALLEN` vs the rig's `telescope_configuration` (±5 %, because header FOCALLEN is nominal while the config is measured), and filter line names vs the rig's `rig_filter_slot` loadout. Exactly one survivor → attributed; zero or several → NULL (never guess). Same-model camera twins are detected by `(manufacturer_id, sensor_id)` equality rather than name similarity (`camera` is `UNIQUE(manufacturer_id, model_name)`, so twins are distinctly-named rows); a twin never eliminates a rig, and once the rig is attributed the frame's camera is **corrected to that rig's actual body** — closing the v0.39.0 two-camera limitation. The pass also back-fills `filter_id` from `filter_name_hint` and **re-keys auto-sessions to (rig, observing night)**, sweeping emptied sessions, so dual-rig nights finally split into per-rig sessions. It runs automatically after every scan *and* on demand via `POST .../resolution/rerun` (shared single-flight lock), so the two paths can't drift. (2) **Unresolved-alias review** on Admin — the global `unresolved_equipment_observation` queue becomes a "map this header string to equipment" screen with confirm/dismiss and a pending count; confirming inserts the alias, and the resolver still never auto-confirms. Mounts routinely appear here because `TELESCOP` often carries one (e.g. `ZWO AM5`). (3) **Per-frame manual override** of rig/camera/filter, protected by migration 0041's `rig_source`/`camera_source`/`filter_source` (`auto`|`user`) — automated re-runs skip `user` fields and the re-scan UPDATE guards them with SQL `CASE`. Explicit null means "none", `reset_to_auto` hands a field back; any existing rig is accepted (the user is the authority), a filter on a calibration frame is 422, and a rig override re-keys that frame's session. Also: canonical line names no longer enter the unresolved queue — a global "Ha → one filter" alias would be wrong for any dual-rig user, and the queue would re-nag on every scan.

- **Route:** Project detail → "Catalog" tab (Re-run resolution button + per-card attribution edit); Admin → Equipment Aliases
- **API:** `GET /api/equipment/unresolved`, `POST /api/equipment/unresolved/{id}/confirm`, `DELETE /api/equipment/unresolved/{id}`; `POST /api/projects/{id}/resolution/rerun`; `PATCH /api/projects/{id}/catalog/frames/{id}/equipment`
- **Key backend:** `services/rig_attribution.py` (`load_project_rigs`, `match_rig`, `rerun_resolution`), `api/equipment_aliases.py`, `api/ingest.py`, `db/migrations/0041.sub_frame_attribution_source.sql`; `ensure_session` / `observing_window_utc` moved into `services/ingest_sessions.py` so the service layer stays free of `api/` imports
- **Key frontend:** `components/admin/EquipmentAliasesSection.tsx`, `components/projects/EquipmentOverrideDialog.tsx`, `components/projects/CatalogCards.tsx` (resolved-vs-hint filter chip, rig chip), `api/equipmentAliases.ts`
- **Tests:** `tests/test_rig_attribution.py` — 33 tests (pure `match_rig` elimination incl. twins + tolerance boundaries; end-to-end attribution + filter back-fill + session keying; two-camera disambiguation by focal length and under a user-set rig; ambiguous stays NULL; re-run idempotency; alias confirm/dismiss incl. 409/404; line-name confirm 422 + migration/map sync; single-filter rig loadout + elimination; override protection across both re-run and re-scan; reset-to-auto; filter-on-dark 422)
- **Known limitations:** attribution needs `project_rig` populated — an unassigned project catalogs but never attributes; a rig whose `telescope_configuration` focal length is absent contributes no focal signal; the re-resolution pass is O(frames) in SQL (one resolver round-trip per frame) and reads each frame's full `fits_header_json` for three values — fine at current scale, but memoization + hint columns are queued for v0.41.1

---

## Schema state

Current migration: **0042** (`purge_line_name_observations`). 42 migrations total (`0001`–`0042`).
