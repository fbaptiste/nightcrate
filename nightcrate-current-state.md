...
**v0.40.1 Sky Position dial + moon-separation correctness.** Detail-panel "Sky position" section now pairs the altitude-vs-time chart with a **sky-position dial** (`SkyPositionView` hosts a Flat/3D toggle, persisted in `localStorage`, default Flat): `SkyDomeChart` (top-down polar alt-az, altitude rings labelled 90°→0°) and `SkyDome3DChart` (oblique shaded cyan-glass hemisphere on a dark backdrop over a tan ground plane, bold cyan equator, faint alt-az grid, per-object azimuth meridians + foot-ticks, depth-shaded dotted night paths, white haloed compass labels, tilted + rotated). Both driven live by the altitude chart's scrubber (`SkyPositionGraph` gained `onActiveIndexChange`); shared `SkyDialReadout`. Backend added `moon_azimuth_deg` to the sky-track. **Correctness fixes shipped alongside:** (1) Moon–target **separation** was corrupted by passing the distance-bearing `get_body` Moon into an ICRS `.separation()` — fixed at all 3 call sites via `astronomy.direction_only()` (feeds the planner list, scoring, and annual chart); (2) chart **"today" markers** were off-by-one in the evening — centralised `astronomy.tonight_date()`, backend stamps `today` on annual-hours + wishlist-calendar responses; (3) **switch-to-object** detail panel rendered blank facts — `/score` now returns the visibility summary, and the plan editor always requests `include_moon`.
...
- **Schema:** migrations 0029 (`project` + `project_image`), 0031 (drop UNIQUE on name), 0032 (`project_thumbnail`), 0033 (drop `staged` column — save-as-you-go), 0034 (`project_solve` + `project_dso`), 0035 (`project_rig` + `project.location_id` + `project_session` + `project_filter_goal` — v0.38.0), 0036 (`project_target` — persistent main targets; backfills from existing solve mains), 0043 (`sub_frame` classification-source guards), 0044 (`project_session.filter_label`; clears the dormant equipment FKs), 0045 (drops `project_filter_goal` + the alias tables). **`project_filter_goal` no longer exists** — per-filter goals were removed in v0.41.1
...
Point a project at one or more source folders and catalog every file in them, classified by FITS `IMAGETYP` (never folder names). Migration 0037 lands the full imaging-core schema: `session` (auto rig-night grouping, distinct from the manual `project_session`), `sub_frame` (content-hash-identified core atom, all equipment FKs nullable), `processed_image` (masters/stacks), `file_location` (every cataloged file, any category), `ingestion_run` (provenance), `project_source_folder`, the empty guiding/session-event/session-log/autofocus tables (forward-FK placeholders for v0.43/0.44), and the calibration-matching + integration views. The ingest pipeline (pure `services/ingest_{scanner,classify,sessions,models}.py` + `api/ingest.py` boundary) walks folders, parses headers in a `ProcessPoolExecutor`, resolves equipment (v0.39.0 resolver), content-hash UPSERTs (idempotent re-ingest, per project), forms noon-to-noon sessions, and assigns lights to the project target. **Each project owns its files (migration 0040):** `sub_frame`/`processed_image`/`file_location` carry `project_id NOT NULL` with per-project identity (`UNIQUE(project_id, content_hash)` / `UNIQUE(project_id, path)`) — the same file cataloged into two projects is two independent rows, and every catalog query is a plain `project_id` match (no more path-prefix scoping). Lights ingest even when the filter doesn't resolve — `sub_frame.filter_name_hint` keeps the raw `FILTER` (no light-needs-filter CHECK); v0.41.0's attribution pass back-fills `filter_id` from the hint once the rig is known. Read-only "Catalog" tab has 7 category sub-tabs (Lights/Darks/Flats/Bias/Dark Flats/Masters/Others, counts in labels) over a reusable full-height grid: frame tabs from `catalog/frames?frame_type=`, Masters from `catalog/masters` (processed images), Others from `catalog/others` (logs/pxiproject/other files + unknown subs with a Type column). Per-row thumbnails are served by a dedicated cheap endpoint (`services/catalog_thumbnail.py` decimates the raw array before rendering — a 26 MP sub thumbnails in ~0.07 s — applies the AutoSTF stretch in **pure numpy** because the mlx GPU backend is not thread-safe and segfaults under the grid's concurrent renders; caches a small JPEG on disk by content hash; concurrency-bounded, lazy-loaded). Dark-flats are inferred by exposure (a `DARK` matching a flat's exposure, not a light's, becomes `dark_flat`); darks/dark-flats/bias are stored filterless. Editing arrived in v0.41.0 (equipment overrides + re-run resolution); `frame_type` corrections, session/target reassignment, and bulk ops are v0.41.1.

**Catalog UI (final form, after real-data testing).** The read-only view is a **card-based infinite-scroll list** (not a DataGrid — MUI X Community caps the DataGrid at 100 rows/page, which hid most frames). Each card: thumbnail, filename + full directory path (start-truncated on overflow, full path in tooltip), filter/object, exposure(s)/gain/set-temp/binning, **pixel dimensions + file size**, and the date in the project's display timezone. Filter pills filter the list (count on Lights, +integration on Flats); Flats sort by filter, others by path so raw vs PixInsight stages bunch. Masters show filter (FILTER → `line_name`), dimensions, and **integration time** (`processed_image.total_exposure_seconds`, migration `0039`, from `PCL:TotalExposureTime`); XISF pixel dimensions are surfaced from the geometry attribute. **Re-scan is per-folder and auto-on-add; removing a folder purges its cataloged items** (orphan sweep of subs/masters/file_locations + emptied auto-sessions).
...
- **Key backend:** `api/ingest.py`, `services/ingest_scanner.py`, `services/ingest_classify.py`, `services/ingest_sessions.py`, `services/ingest_models.py`, `services/catalog_thumbnail.py`, `db/migrations/0037.imaging_core.sql` + `0038.sub_frame_filter_hint.sql` + `0039.processed_image_total_exposure.sql`
- **Key frontend:** `components/projects/ProjectCatalogTab.tsx` (7 sub-tabs) + `components/projects/CatalogCardList.tsx` (infinite scroll) + `components/projects/CatalogCards.tsx` (cards/formatters), `api/projectCatalog.ts`; folder picker is the shared `components/fits/FileBrowser` in `directoryMode` (same browser as Image Analyzer / PHD2)
- **Tests:** `tests/test_imaging_schema.py` (20), `tests/test_ingest.py` (47, incl. thumbnail service + endpoint) — 90%+ module coverage
- **Known limitations:** single-flight ingest (one at a time); standard image exports (TIFF/PNG/JPG) parked as "other" until gallery promotion in v0.41.3. (Rig attribution + filter back-fill landed in v0.41.0 — see below.)

### Catalog Corrections + Derived Sessions (v0.41.1)

**Status:** `[shipped]`

Two things at once: a **strip-out** and a **new capability**.

**Removed — all automatic equipment identification.** v0.39.0's FITS-header→equipment-row resolver, v0.41.0's rig-attribution pass, the global Admin alias review queue, and the per-frame equipment override are gone (`services/equipment_resolver.py`, `services/rig_attribution.py`, `api/equipment_aliases.py`, `EquipmentAliasesSection.tsx`, `EquipmentOverrideDialog.tsx`, the "Re-run resolution" button, `POST .../resolution/rerun`, `PATCH .../equipment`, and the seed loader's `_alias` table path with its three empty CSVs). The reason: per-frame camera/telescope/filter attribution was a large, fragile surface answering a question the project already answers — **a project's assigned rigs (`project_rig`) are the equipment context.** `sub_frame.filter_name_hint` (the FITS `FILTER` header, normalized to a display short-form) is now the only filter fact a cataloged frame carries; it drives the catalog filter pills, the card's single outlined filter chip, and the session-derivation grouping. Migration 0046 then dropped every equipment FK on `sub_frame` / `processed_image` except `rig_id` — see the folder rig tag below. Per-filter **integration goals** went too (`project_filter_goal`, `PUT /integration/goals`, `FilterGoalsEditor.tsx`) — the integration bars are a totals read-out now, no goal tick and no percentage.

**Added — a declared rig per source folder, and header-keyed calibration matching.** `project_source_folder.rig_id` (migration 0046) is the one equipment fact the ingest records, and the **user** declares it: tag a bound folder with the rig that shot it, and every frame beneath inherits it into `sub_frame.rig_id`. Sessions key on (project, observing night, rig) so a simultaneous dual-rig night splits into two sessions again; `PATCH .../folders/{id}` re-tags already-cataloged frames and re-keys their sessions without a re-scan. Migration 0046 also rebuilt the four calibration views on the facts the FITS header actually gives us — darks on exposure + gain + binning + set-temp ±1 °C, flats on gain + binning + `filter_name_hint`, bias on gain + binning — scoped to project **and** rig (they were never project-scoped before). They had gone silently dead: the originals compared `d.camera_id = l.camera_id` with a plain `=`, and once every `camera_id` was NULL, `NULL = NULL` is NULL rather than TRUE, so every view returned zero rows. `integration_time_per_project_filter` was dropped outright — integration has one source of truth now.

**Added — derived imaging sessions.** `POST /projects/{id}/sessions/derive` → `services/session_derivation.py` (pure service, caller owns the transaction) rebuilds the project's `source='auto'` sessions from its cataloged lights, one row per **(observing night, filter, exposure, gain, binning)**, and never touches `source='manual'` rows. Grouping is on the *canonical* filter key so `Ha` and `H-alpha` in one night collapse to one row. **The derive is never automatic** — ingest catalogs frames and stops there, because a project may legitimately hold no subs at all (the user doesn't have them, or doesn't want them in the project) and still keep a full hand-entered session record. Derived rows are **read-only** (409 on PATCH/DELETE, no row actions rendered): editing a row the next derive replaces is a lie; the escape hatch is to correct the frames on the Catalog tab and derive again. Migration 0044 adds `project_session.filter_label` for the header filter name, while `line_name` stays set (canonical, else `'other'`) purely so the existing filter-or-line CHECK holds without a table rewrite; the UI renders `filter_label` first so a derived `L-eXtreme` row doesn't display as `other`. `IntegrationLine.label` became a free string for the same reason. Sessions themselves are now keyed on **(project, observing night)** alone.

**Kept from the original scope.** Frame-**classification** corrections — `frame_type` and target, single and bulk — survive, guarded by migration 0043's `frame_type_source` / `project_target_source` (both values are re-derived on every scan, so without the guards a re-scan would wipe them). And the **embedded Image Analyzer**: `ImageAnalyzerPage` was split into an embeddable `components/analyzer/ImageAnalyzerView`, the module-global Zustand store deleted in favour of per-instance `useState` so the route page and the project overlay can't stomp each other, and `AnalyzerOverlay.tsx` opens any catalog card full-screen with prev/next stepping without leaving `/projects/:id`. The project Plate Solve tab now routes through the shared `PlateSolveDialog` with coordinate + equipment hints instead of firing blind from the file browser.

- **Route:** Project detail → "Sessions" tab (Add session / Derive from subs, source chips, integration bars); "Catalog" tab (corrections + analyzer overlay)
- **API:** `POST /api/projects/{id}/sessions/derive`; `PATCH /api/projects/{id}/catalog/frames/{id}/classification`; `POST /api/projects/{id}/catalog/frames/bulk-classification`. **Removed:** `/api/equipment/unresolved*`, `POST .../resolution/rerun`, `PATCH .../equipment`, `PUT .../integration/goals`
- **Key backend:** `services/session_derivation.py` + `_models.py`, `services/line_names.py` (the bandpass vocabulary + canonicalization, moved out of `api/project_session_models.py` so a pure service can use it), `api/project_sessions.py`, `api/ingest.py`, `db/migrations/0043.sub_frame_correction_source.sql` + `0044.session_derivation.sql` + `0045.drop_equipment_resolution.sql`
- **Key frontend:** `components/projects/ProjectSessionsTab.tsx` (derive action + confirm, source chips, read-only derived rows), `components/projects/IntegrationBars.tsx`, `components/projects/FrameCorrectionsDialog.tsx`, `components/projects/AnalyzerOverlay.tsx`, `components/analyzer/ImageAnalyzerView.tsx`
- **Tests:** `tests/test_session_derivation.py` (48 — pure grain/coercion helpers, replace-auto/keep-manual, canonicalization + `other` fallback, noon-boundary timezone, zero-exposure skip, idempotence, endpoint 404/409, and a test pinning "ingest does not derive"), `tests/test_catalog_corrections.py` (14). Deleted: `test_equipment_resolver.py` (29) and `test_rig_attribution.py` (33)
- **Known limitations:** unbinding a source folder leaves stale derived sessions until the user derives again (deliberate — nothing re-derives on its own; the "last derived" caption makes it visible). Hand-entering a session for a night *and* deriving that night produces two rows that both count toward integration — unguarded, left for v0.42.0
...
- **Equipment (migrations 0005–0006, plus inline edits in v0.12.0):** 12 equipment tables, 10 lookup/reference tables, 5 junction tables, 2 child tables, 1 view, `seed_loader_meta` — fully normalized equipment catalog. `is_mine` column + partial index added to 10 owned equipment tables in v0.12.0. `idx_camera_guide_sensor` added inline to 0006 in v0.12.1.
...
- **Project-owned files (migration 0040, v0.40.0):** rebuilds `sub_frame`, `processed_image`, and `file_location` (same table-rewrite pattern, `legacy_alter_table` to preserve child FKs) to add `project_id NOT NULL` (CASCADE) and per-project identity — `UNIQUE(project_id, content_hash)` / `UNIQUE(project_id, path)`. Each project owns its own row for a file; the same physical file cataloged into two projects is two independent rows (no shared/global identity). Replaces the old indirect `sub_frame → ingestion_run.project_id` scoping and the path-prefix `_project_file_scope` for non-frame files — every catalog query is now a plain `project_id` match. Backfills `project_id` from the ingestion run / session / source-folder prefix; drops rows that resolve to no project. Verified applying on a populated 0039 dev DB (196 subs / 201 files preserved, no NULL project_id).
- **Attribution source (migration 0041, v0.41.0):** ALTER-adds `sub_frame.rig_source` / `camera_source` / `filter_source` (`TEXT NOT NULL DEFAULT 'auto'` CHECK `auto`/`user`). `'user'` marks a manual override that no automated re-resolution pass may clobber; existing rows default to `'auto'` since everything written before v0.41.0 was resolver-written. Only the three user-overridable attributions carry a source column — telescope/config follow from the attributed rig and stay automated. Verified against a populated 0040-state DB as well as a fresh build.
- **Line-name queue purge (migration 0042, v0.41.0):** data-only, no DDL. Deletes **unresolved** `filter` observations whose `normalized_alias` is a canonical line name. Up through v0.40.3 the resolver recorded an observation for any unmatched `FILTER` value and ingest never supplied a `RigContext`, so every scan queued `Ha`/`Oiii`/`Sii`; those rows would surface in the new Admin review queue and invite confirming a **global** alias — wrong for any dual-rig user. Rows a user already resolved are left alone (deleting them would strand the alias they point at). Paired with a 422 guard in the confirm endpoint so a stale client can't recreate one, and a test asserting the migration's SQL list stays in sync with `_LINE_NAME_MAP`. Verified on a populated 0041-state DB.
- **Classification source (migration 0043, v0.41.1):** ALTER-adds `sub_frame.frame_type_source` / `project_target_source` (`auto`|`user`, default `auto`). Both values are re-derived on every scan — `_reclassify_dark_flats` re-runs the heuristic dark → dark_flat promotion project-wide, and `_persist_parsed` writes a NULL target for any project without exactly one main — so without these guards a re-scan would silently wipe a hand correction.
- **Session derivation (migration 0044, v0.41.1):** ALTER-adds `project_session.filter_label` (the header filter name a derived row was grouped on). A plain ADD COLUMN suffices because derived rows also set `line_name` (canonical, else `'other'`), so the table's filter-or-line CHECK still holds — no rewrite. Also clears `session.rig_id` and every equipment FK on `sub_frame` / `processed_image`: nothing writes them from here on, so the partly-populated values (including a handful of manual overrides) are cleared rather than left as half-truths.
- **Equipment-resolution removal (migration 0045, v0.41.1):** drops `project_filter_goal` + the `project_filter_goal_progress` view (per-filter goals removed), the three alias tables + `unresolved_equipment_observation` (alias review queue removed — no inbound FKs, so the drops are unconstrained), and `sub_frame.rig_source` / `camera_source` / `filter_source` (the per-frame override they guarded is gone). `DROP COLUMN` is safe for all three: none is a PK/UNIQUE, indexed, or named in a view or trigger. 0043–0045 verified together on a copy of the populated dev DB (2341 sub frames preserved, `foreign_key_check` + `integrity_check` clean) as well as a fresh build.
- **Header-keyed calibration (migration 0046, v0.41.1):** rebuilds `matching_darks` / `matching_flats` / `matching_bias` / `calibration_coverage` on header facts scoped to project **and rig** (the originals joined `camera_id = camera_id`, which after 0044 was `NULL = NULL` — NULL, not TRUE — so every view silently returned zero rows). Drops `integration_time_per_project_filter` (integration's single source of truth is now the derived `project_session` rows) and the remaining equipment FK columns on `sub_frame` / `processed_image` (`rig_id` stays). Adds `project_source_folder.rig_id`, the user-declared rig tag that frames inherit and that sessions + calibration scope on. Views must be dropped before the columns they name; `session_summary` rebuilt to count distinct `filter_name_hint`. Verified on a populated 0045-state DB.
...
- **FITS equipment resolver** — built in v0.39.0–v0.41.0 and then **deliberately removed in v0.41.1**; not a gap. A project's assigned rigs are the equipment context, and `sub_frame.filter_name_hint` is the only filter fact a frame carries. Do not rebuild it without re-opening that decision.
- **Calibration coverage UI** — the matching views work (re-keyed on header facts in migration 0046, verified against real data), but nothing reads them yet. v0.42.0 puts coverage indicators on the catalog cards.
- **Log ingestion** — N.I.N.A. session logs, ASIAIR logs, PHD2 guiding association into `session_event` / `guiding_sample` / `autofocus_run` (the tables exist, empty, since migration 0037). v0.43.0–v0.44.0.
