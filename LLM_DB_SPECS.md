...
- `thumbnail_cache` (v0.16.0, migration 0017) — metadata for the Target Planner's LRU thumbnail cache. Files live on disk under `APP_DIR/thumbnails/`; rows carry the `dso_id` FK (cascade-delete), `variant` (`list`/`detail`), dimensions, `source` (`dss2_color`/`dss2_red`/`dss2_blue`/`placeholder`), byte size, fetched/last-access timestamps, and a nullable `fetch_error` (non-null rows are backoff sentinels that expire after 1 hour). Not seed-loaded — populated entirely by runtime fetches from CDS Aladin's hips2fits.
