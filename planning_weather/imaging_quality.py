...
composition only drives an advisory flag (HIGH_CLOUD_ONLY) — see the design
notes for why no partial credit is given to high cloud.
"""
...
class Role(Enum):
    """How a factor enters the score. The UI can group/colour rows by this."""
...
class Flag(Enum):
    ...
    WIND_GATE = "wind_gate"
    OVERCAST = "overcast"
    HIGH_CLOUD_ONLY = "high_cloud_only"   # obscuration is (almost) all >= 8 km: lower forecast skill
...
# --------------------------------------------------------------------------- #
# Tunables. Each is a judgement call unless noted; see design notes.
# --------------------------------------------------------------------------- #
...
"""NORMAL mode only: quality multiplier at moon sub-score 0 (full moon, up all
hour). 1.0 at moon sub-score 100. Broadband imaging of faint targets under a
full moon keeps roughly a third of its value; bright targets do better, which
is why this is a floor and not a gate. Target–moon separation is the target
...
def effective_cloud_fraction(
    *,
    ...
    inconsistent feed and monotonic in every input. If the total is missing,
    it is estimated from the layers with random overlap, 1 - prod(1 - f_l),
    which is the conservative (higher) estimate.

    Raises ValueError if no cloud figure at all is available: an hour cannot
    be scored without cloud data, and silently treating 'unknown' as 'clear'
    would be worse than failing.
    """
    total = _pct(cloud_cover)
    layers = tuple(v for v in (_pct(cloud_cover_low), _pct(cloud_cover_mid),
                                _pct(cloud_cover_high)) if v is not None)
    flags: list[Flag] = []
...
def score_hour(
    *,
    ...
    cloud_cover, cloud_cover_low/mid/high
        Percent. See `effective_cloud_fraction` for how they combine.
    seeing, transparency, wind_calm, moon
        0–100 sub-scores, higher is better, derived upstream. `moon` must be
        the value for *this hour* (moon altitude and illumination at the
        hour), not a nightly figure, or a post-moonset clearing scores wrong.
    darkness
        Fraction of this hour inside astronomical darkness, 0–1. A location
        with no astronomical darkness passes 0 for every hour. How the caller
        treats nautical twilight for narrowband is the caller's decision.
    precipitation_mm, precipitation_probability
        Any precipitation amount closes the hour. Probability ramps the gate
        down across PRECIP_PROBABILITY_RAMP. Either may be None.
    wind_speed_kmh
        Sustained 10 m wind for the gate; None disables the gate (the
        wind_calm quality term still applies).
    temperature_c, dew_point_c
        Used only to raise DEW_RISK. Never affect the score: dew heaters make
        this a preparation issue, not a go/no-go one.
    """
    flags: list[Flag] = []
    ...
    if precipitation_mm is not None and precipitation_mm > 0.0:
        ...
        precip_value = 100.0 - p  # shown as "dry %" so every row reads higher-is-better
    if precip_gate <= 0.0:
        flags.append(Flag.PRECIPITATION)
    factors.append(FactorScore("precipitation", Role.GATE, precip_value, precip_gate,
                               applied=precip_value is not None))
    ...
    factors.append(FactorScore("wind_gate", Role.GATE,
                               None if wind_speed_kmh is None else 100.0 * wind_gate,
                               wind_gate, applied=wind_speed_kmh is not None))
...
def _label(score: int, availability: float) -> str:
...
def expected_useful_hours(results: Iterable[ImagingQuality]) -> float:
    """Night-level summary: sum of hourly score/100.

    Because score = availability * quality, this reads as 'equivalent hours of
    perfect data' and is the number to put next to the go/no-go decision.
    Hours outside darkness contribute 0 automatically.
    """
    return sum(r.score for r in results) / 100.0
