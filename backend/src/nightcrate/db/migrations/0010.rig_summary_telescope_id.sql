-- Recreate rig_summary view to include telescope_id.
--
-- Migration 0009 originally did not expose telescope.id; the Equipment tab
-- needs it to fetch the full telescope record. 0009 was edited in place per
-- the pre-release migrations policy, but databases that had already applied
-- 0009 before the edit need this explicit drop+recreate.
