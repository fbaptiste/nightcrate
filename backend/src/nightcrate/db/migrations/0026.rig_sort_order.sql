-- depends: 0025.target_wishlist
-- Add user-defined sort order to rigs.

ALTER TABLE rig ADD COLUMN sort_order INTEGER NOT NULL DEFAULT 0;

-- Backfill: preserve rough creation order by using the row id.
UPDATE rig SET sort_order = id;

-- Rebuild rig_summary to expose sort_order.
