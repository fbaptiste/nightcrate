-- v0.41.1 — expose rig.source through rig_summary.
--
-- The UI needs to tell a catalog rig from one the user built, because the two
-- want different controls: a user's rig can be retired, while an all-in-one
-- smart telescope is a catalog entry that is either yours or not. Retiring one
-- produced a confusing third state — still seeded, still in the catalog
-- conceptually, but filed under "Retired Rigs".
--
-- Same view rebuild as 0048 and, before it, 0026: SQLite freezes a view's
-- column list at creation.
