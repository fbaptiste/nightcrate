...
# Generated per-frame sidecars: one file per registered sub, carrying processing
# metadata rather than data. Cataloging them buries the real contents — a project
# with 600 subs gains ~1,250 of these rows — and there is nothing a user can do
# with one. They are outputs of a processing step and are regenerated by re-running
# it, so nothing is lost by not tracking them.
#
#   .xnml  PixInsight "XML Local Normalization Data" — LocalNormalization's
#          per-sub model, applied by ImageIntegration.
#   .xdrz  PixInsight "XML Drizzle Data" — StarAlignment's registration transform,
#          used by DrizzleIntegration to stack from uninterpolated pixels.
#   .xpsm  PixInsight process/state blob.
#
# NOTE this is "generated sidecar", NOT "not an image": logs are not images and
# are cataloged deliberately (they are the v0.43/v0.44 ingestion arc), and so is
# the .pxiproject itself.
_SIDECAR_EXTENSIONS = frozenset({".xnml", ".xdrz", ".xpsm"})


def is_sidecar(name: str) -> bool:
    """True for a generated per-frame sidecar that should never be cataloged."""
    return _ext(name.lower()) in _SIDECAR_EXTENSIONS
