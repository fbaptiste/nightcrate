...
def _cards_to_dict(cards: list[dict]) -> dict[str, str]:
    """Convert header card list to a {KEY: value} dict (first occurrence wins)."""
    raw: dict[str, str] = {}
    for card in cards:
        if card["key"]:
...
async def _do_solve(
    ...
    source, file_type, image_index, _cache_key = resolve_path(image_path)
    cards = _read_header_cards(source, file_type, image_index, hdu)
    hints = _extract_hints(cards)

    raw_header = _cards_to_dict(cards)
