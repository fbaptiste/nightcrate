...
| `thumbnail_cache` (v0.16.0, migration 0017) | Target Planner LRU thumbnail cache metadata. Files live on disk under `APP_DIR/thumbnails/`. Rows carry `dso_id` FK (cascade-delete), `variant` (`list`/`detail`), dimensions, absolute `file_path`, `source` (`dss2_color`/`dss2_red`/`dss2_blue`/`placeholder`), `bytes`, `fetched_at`, `last_access_at`, and a nullable `fetch_error` (non-null rows are backoff sentinels that expire after 1 hour). `UNIQUE(dso_id, variant, width, height)`. Index on `last_access_at` drives LRU eviction. |
