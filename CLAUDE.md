...
- `services/thumbnails.py` + `services/hips_client.py` — CDS Aladin hips2fits fetch (DSS2 Color → DSS2 red fallback), disk cache under `APP_DIR/thumbnails/`, LRU eviction via `_evict_lru` on every successful fetch. Miss returns a 1×1 transparent PNG with HTTP 202 + enqueues a background `asyncio.create_task` (deduped via `_in_flight` map keyed by full `ThumbnailKey`, capped via `_fetch_semaphore = 4`). **Optional long-poll:** callers passing `wait_timeout_s > 0` wait on the background task under `asyncio.shield(asyncio.wait_for(...))` and return the real image in the same round trip — eliminates the "CDS latency + next-poll cadence" overhead pollers pay. Double-failure inserts a `fetch_error` sentinel row that expires after 1 hour — no refetch storms. Metadata in `thumbnail_cache` (migration 0017, 0018 adds FOV descriptor columns + rig-dependent variants, 0019 adds `center_ra_deg_x1000` / `center_dec_deg_x1000` for panned simulator tiles), files on disk. Pass B variants: `rig_framed` (180×180, extent = rig major-axis FOV exactly) and `fov_simulator` (800×800, extent = `max(rig_diag × 1.5, obj × 2)`, tiled 5×5 around the target). Pass A `detail` widened to 800×800 / 3.5×-1.0° extent. `sync_orphan_files()` runs in the app lifespan to sweep on-disk JPEGs not referenced by the cache (needed because migration 0018 wipes the rows but SQL migrations can't unlink files).
...
- `api/planner.py` + `api/planner_models.py` — `/api/planner/targets` (filter + sort + paginate in memory against the cached snapshot), `/targets/{id}/sky-track`, `/thumbnails/{id}?variant=...` (optional `wait_ms` long-poll window + `center_ra_deg` / `center_dec_deg` for panned simulator tiles), `/thumbnails/cache/{clear,stats}`, `/dsos/in-region` (annotation overlay).
- Frontend: `pages/PlannerPage.tsx`, `components/planner/{ThumbnailCell,SkyPositionGraph,PlannerDetailPanel,FovSimulator,DsoAnnotationOverlay,DsoAnnotationPopover}.tsx`, `lib/dsoAnnotations.ts`, `stores/thumbnailCacheStore.ts`, `api/planner.ts`. `thumbnailUrl()` returns a full `/api/planner/thumbnails/...` URL for direct `<img src>` use (bypasses `apiFetch`); takes `ThumbnailUrlOptions` (`fovMajorDeg`, `fovMinorDeg`, `centerRaDeg`, `centerDecDeg`, `generation`, `waitMs`) and folds `_activity` into the query so thumbnail requests show up in the Activity Console.
- `ThumbnailCell` detects the 1×1 placeholder via `naturalWidth <= 1` and retries with a backoff schedule (400 / 900 / 1500 / 2500 ms). `onError` swaps to a neutral icon for 204 / error cases. Accepts an `aspectRatio` prop (used by the "In my rig" column to crop the square 180×180 image via `object-fit: cover` into a sensor-shaped bounding box) and a `waitMs` prop (simulator tiles pass 4000 on the first attempt; retries use 0 so a slow CDS can't stack 4 s connection holds). An `imgRef` + `useLayoutEffect([src])` safety net catches the cached-`<img>` race where the browser serves `src` synchronously from HTTP cache and `onLoad` fires before React's listener is attached.
- `FovSimulator` — Pass B detail-panel hero when a rig is selected. Orange rotatable sensor rectangle over a pannable tile mosaic of DSS2 background tiles fetched from hips2fits. Drag-to-rotate, numeric input, arrow keys (±5° / ±1° with Shift), R resets. Tile grid starts at 1×1 (centre only — single CDS fetch on first open), promotes to 3×3 on the centre's `onReady`, then 5×5 two seconds later. Call site (`PlannerDetailPanel`) carries `key={dsoId:fov_major:fov_minor}` to remount on target/rig change; without it, the prior target's `gridN=5` state would bleed into the new DSO's first render and stampede 25 CDS fetches before the reset useEffect could run. A faint dashed grid (opacity 0.14, stroke + dash counter-scale by zoom) signals the mosaic so per-tile seams read as intentional.
- Annotation overlay (`DsoAnnotationOverlay` + `lib/dsoAnnotations.ts`) uses **per-tile gnomonic projection** via `projectRaDecToTilePixel`: each annotation picks the tile whose tangent is nearest on the sphere (four candidates around floor/ceil of col/row), then projects on *that* tile's tangent. A single grid-wide projection can't match 25 independent tangent planes; a flat plate-carrée approximation drifts ≈47 px on the dec axis at dec ≈+69°. Shared `tileCenterAt` helper keeps the tile-layout formula in lockstep between `computeTiles` (simulator) and the overlay (annotations). A simpler `projectRaDecToPixel` still exists for single-tangent callers.
...
- Thumbnail variants: `list` = 180×180 at `max(maj_axis × 1.5, 0.1°)`; `detail` = 800×800 at `max(maj_axis × 3.5, 1.0°)`; `rig_framed` = 180×180 at `fov_major_deg`; `fov_simulator` = 800×800 at `max(fov_major, fov_minor) / 0.5`.
- `thumb_dir()` ensures `APP_DIR/thumbnails/` exists on first access — don't precheck in callers.
- `tileCenterAt` in `lib/dsoAnnotations.ts` is the sole source of the simulator's plate-carrée tile-centre arithmetic. `FovSimulator.computeTiles` and `projectRaDecToTilePixel` both call it; don't duplicate the formula elsewhere (annotation alignment depends on byte-for-byte agreement between tile placement and projection).
