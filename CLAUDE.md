...
## PHD2 Guide-Log Analyzer (v0.22.0 Pass A + v0.23.0 Pass B + v0.24.0 Pass C + v0.25.0 Pass D-1 + v0.26.0 Pass D-2)

Part of the ten-version arc (v0.22.0 → v0.34.0 + roadmap tail). Full spec: `docs/nightcrate-phd2-analyzer-spec-v4.md`. Passes A–D-2 are live: parser + time-series chart + calibration plot + scatter plot + per-section / viewport / selection summary metrics with PHDLogViewer-aligned formulas, event list with click-to-jump, multi-additive range selection / exclusion, copy-stats, recent-files dropdown, four-tab layout (Guiding / Spectrum / Dispersion / Data), session-info panel, vertical scale sliders flanking the time-series chart, and an FFT spectrum view with worm-period markers. Interpretive diagnostics start in v0.31.0; persistence lands in v0.29.0. Sample log for local testing: `sample_data/session_logs/ASIAir/PHD2_GuideLog_2026-03-07_193345.txt`.
...
- `api/phd2.py` + `api/phd2_models.py` — `POST /api/phd2/parse` accepts an optional `rig_id`, `GET /api/phd2/cache/stats`, `POST /api/phd2/cache/clear`. TTL cache keyed by `(path, mtime_ns, size, rig_id)` with per-key locking (mirrors `api/images.py`). 120 s TTL, 8-entry cap. Rig context drives the worm-marker overlay only — the FFT itself is rig-agnostic.
- `services/phd2_fft.py` (v0.26.0) — pure service implementing spec v4 §6.1 end-to-end. Public entry `compute_section_fft(samples, *, pixel_scale, trace="ra") -> FftResult | None`. Pipeline: filter to `Include()` frames, MIN_ENTRIES=12 + IQR/median > 0.20 cadence guard + constant-data guard (each surfaces as a structured `skip_reason: "too_short" | "non_uniform_cadence" | "constant_data"`), least-squares drift subtraction, Akima spline resample to uniform cadence (`scipy.interpolate.Akima1DInterpolator`), Hamming window (`np.hamming`), `numpy.fft.rfft`, `4/N` amplitude scale (matches PHDLogViewer's `AnalysisWin.cpp` line 395 — over-estimates true peak amplitude by ~8% but cross-tool consistency is the v0.26.0 acceptance criterion). Peak detection: MAD-σ threshold `median(amp) + 3·1.4826·MAD(amp)`, ±5% period dedup, top-5 by amplitude. **Backend dep:** `scipy>=1.11` (BSD-3-Clause).
- `_build_worm_marker(rig_info, fft_ra)` in `api/phd2.py` — three-branch overlay: (1) rig with `mount.drive_type` matching "worm" (case-insensitive substring on the existing free-form text — controlled vocabulary lands in v0.28.0) AND `mount.worm_period_seconds` set → vertical marker at the worm period, callout chip when a peak is within ±5%; (2) heuristic fallback (no rig, non-worm mount, or no worm period) → largest peak in `[300, 800] s` above 0.5″, marker labelled "uncertain"; (3) neither → no marker. Migration `0024.mount_worm_period.sql` adds `mount.worm_period_seconds REAL` and rebuilds the `rig_summary` view to expose `mount_drive_type` + `mount_worm_period_seconds`. Worm-period seed values populated for the worm mounts in `data/seed/mount.csv` (e.g. EQ6-R Pro 479, AVX 480, GEM28/CEM70 600, CEM26 287); strain-wave + direct-drive rows blank (proper rig-aware support for those lands in v0.28.0).
- **FftChart** (`components/phd2/FftChart.tsx`, v0.26.0) — D3 SVG dual-trace (RA blue, Dec orange) chart with log-log axes (period in seconds, amplitude in arcsec), seeing-band shading at < 5 s, top-5 peak dots (subtle 3.5 px radius + half-opacity white outline), worm-marker overlay (vertical dashed line + `mount`/`heuristic` chip), X-axis tick rotation -30° to prevent overlap, and a `<clipPath>` covering the inner panel so log-scale values below the Y-axis floor stay visually contained. **Always-on hairline + tooltip:** `handleMouseMove` tracks the cursor anywhere inside the panel rect, drops state when the cursor leaves; the hairline locks horizontally to a peak's period when within ±8 px (snap is X-only — the user's mental model is "the line is near the peak", not "the cursor is near the dot"); snapped tooltip shows full Period / Amplitude / Peak-to-peak / RMS readout with the heading colored to match the trace, free-cursor tooltip shows period + nearest-bin amplitudes per visible trace. Tooltip Y is anchored at a fixed `MARGIN.top + innerH * 0.35` for both states so it never bounces when the hairline locks onto a peak; height auto-resizes by content via `lineCount * 18 + 24`. Inner Box uses `'& .MuiTypography-root': { fontSize: 'inherit', lineHeight: 'inherit' }` so MUI's `caption` variant doesn't override the parent's font math. Legend RA/Dec toggles mounted independently of the chart body so the empty-trace state still shows them.
- **RigSelectBar** (`components/phd2/RigSelectBar.tsx`, v0.26.0) — MUI `<Select>`-based picker with first-class "No rig" option. Persists per-log via the `phd2RecentFiles` localStorage store (`RecentFile.selectedRigId?: number | null` + `setRecentFileRig(path, rigId)`). Reopening a log re-applies the user's rig choice; deleted-rig fallback renders the "No rig" option visually while leaving the stored ID alone until the next change.
- Frontend — `pages/Phd2AnalyzerPage.tsx`, `components/phd2/{TimeSeriesChart,CalibrationPlot,ScatterPlot,FftChart,RigSelectBar,StatsPanel,EventList,WarningsDrawer,SectionNavigator,SectionInfoPanel,SectionDataTab}.tsx`, `lib/phd2GuidingMetrics.ts`, `api/phd2.ts`. D3 + SVG for the time-series, calibration, scatter, and spectrum charts (templates from `planner/SkyPositionGraph.tsx` and `planner/BestTimeOfYearChart.tsx`). RA = blue, Dec = orange — colorblind-safe palette from `lib/rigColors.ts`. Page tabs: **Guiding** (TimeSeriesChart + EventList) / **Spectrum** (FftChart, v0.26.0) / **Dispersion** (ScatterPlot) / **Data** (SectionDataTab). Section / Viewport / Selection summary `StatsPanel`s and the `SectionInfoPanel` live in the left nav under the Section list. RigSelectBar lives in the page toolbar so the rig choice is global; selection re-runs the parse with `rig_id` so the FFT cache key separates rig variants.
...
- v0.26.0 Pass D-2 — Spectrum tab v4-conformance + worm-period markers (Hamming, 4/N normalization, Akima spline via scipy, MAD-3 threshold, always-on hover hairline with horizontal-only snap-to-peak). **Shipped.**
