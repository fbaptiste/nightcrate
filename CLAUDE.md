...
- **Unguided-RA reconstruction** (v0.27.0, spec §6.2) is the PHDLogViewer recurrence `move = raraw − prev_raraw − prev_raguide`. **Mount-kind-agnostic** — AO frames with valid star data accumulate alongside Mount frames; the AO's correction lives in `ra_guide_px` exactly like a Mount pulse. Output is 1:1 with `section.samples` with `None` at DROP-frame indices so the chart breaks the line cleanly across gaps without losing the recurrence (the next valid frame's `move` correctly spans the gap). The TIME-SERIES overlay shows the FULL section's cumulative drift as-is; the SPECTRUM filters to settle-excluded samples and runs the v4 §6.1 pipeline (drift subtraction → Hamming → Akima → MAD-σ peaks). Both share the same teal dashed visual styling for cross-tab continuity.
