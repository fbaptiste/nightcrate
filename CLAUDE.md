...
- **Dark-flats are inferred by exposure, not header.** Capture software (ASIAir) labels dark-flats just `IMAGETYP=DARK` — indistinguishable from real darks per-file. After ingest, `api/ingest.py:_reclassify_dark_flats` (project-scoped, idempotent) promotes a `dark` → `dark_flat` when its exposure matches a flat in the project and not any light (rounded to ms). Real darks (matching the lights' exposure) and master darks (`processed_image`) are untouched.
...
- **NO light-needs-filter CHECK; `sub_frame.filter_name_hint` holds the raw `FILTER`.** At v0.40.0 a light's `filter_id` is routinely NULL (no rig context → resolver can't scope line names; aliases empty). Ingest must never fail on partial equipment — every equipment FK is nullable. v0.41.0 maps `filter_name_hint` → physical filter + back-fills.
- **Pure services + one DB/HTTP boundary.** `ingest_scanner.py` (walk + picklable `parse_image_file` ProcessPool worker), `ingest_classify.py`, `ingest_sessions.py` (noon-to-noon observing-night grouping in the site `geo_timezone`; dual-rig = separate sessions, NULL-rig is its own bucket) — none import from `api/`. `api/ingest.py` owns the transaction, runs the resolver, content-hash UPSERTs, forms sessions, writes provenance. Single-flight ingest (one at a time, 409 if busy).
- **Idempotent re-ingest** keyed on `content_hash` (UPSERT). Re-scan updates rows; counts never grow. **Tests must give synthetic FITS unique pixel data** — byte-identical files correctly dedupe to one row.
...
- **Read-only catalog UI**: `ProjectCatalogTab.tsx` (5th project tab) — folder binding uses the **shared `components/fits/FileBrowser` in `directoryMode`** (same browser as Image Analyzer / PHD2 — do NOT build a separate folder picker), scan button, then **7 category sub-tabs** (Lights/Darks/Flats/Bias/Dark Flats/Masters/Others, count in each label) over a reusable `CatalogGrid`. Frame tabs → `catalog/frames?frame_type=`; **Masters** → `catalog/masters` (processed_image, Type/filter/NCombine); **Others** → `catalog/others` (file_location pxiproject/log/other + unknown-type subs, with a Type column). Grid fills container height (`flex:1` in a `calc(100dvh - …)` column). No editing (v0.41.0).
