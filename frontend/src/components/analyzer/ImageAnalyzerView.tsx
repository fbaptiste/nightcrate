...
import { headerNumber, headerValue, parseHeaderCoord as parseHeaderCoordShared } from "./analyzerHelpers";
...
export interface ImageAnalyzerViewProps {
  /** Controlled. "" means nothing is open. Changing it resets per-file state. */
  path: string;
  /** Label for virtual/project paths (drives the status bar + activity label). */
  displayName?: string;
  /** Toolbar slot, left. The standalone page puts Browse / path / Open here;
   *  the project overlay puts the filename + prev/next there instead. */
  toolbarStart?: React.ReactNode;
  /** Toolbar slot, right (the overlay's close button). */
  toolbarEnd?: React.ReactNode;
  /** Rendered instead of the tabs when `path` is empty. */
  emptyState?: React.ReactNode;
  /** Query failures bubble here so only the visible host shows a snackbar. */
...
export default function ImageAnalyzerView({
  ...
  emptyState,
  onError,
}: ImageAnalyzerViewProps) {
  const queryClient = useQueryClient();
  // Per-file state. This was a module-level Zustand store until v0.41.1, purely
  // so it would "survive navigation" — but AppShell's PersistentPages mounts
  // this page once and only ever toggles `display`, so it never unmounts and
  // local state survives identically. Keeping it local is what lets a second
  // instance (the in-project analyzer overlay) have its own state by
  // construction. NOTE: that depends on "/image-analyzer" staying in
  // PERSISTENT_ROUTES — give it a real route element and this state resets on
  // every navigation.
  ...
  const annotationQuery = useQuery({
    // Fingerprint the WCS, don't just record that one exists: a header solution
    // and a plate-solve solution for the same (path, hdu) would otherwise share
    // one cache entry, so solving would show the header's annotations (and, with
    // two view instances open on the same file, they'd alias each other).
    queryKey: [
      "annotations",
      activePath,
      selectedHdu,
      solvedWcs
        ? `solved:${solvedWcs.crval1},${solvedWcs.crval2},${solvedWcs.crpix1},${solvedWcs.crpix2}`
        : wcsQuery.data
          ? "hdr"
          : "none",
    ],
  ...
  useEffect(() => {
    if (extensionsQuery.isError) {
      const msg = String(extensionsQuery.error);
      setErrorMsg(msg);
      onError?.(msg);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- onError identity is the host's business
  }, [extensionsQuery.isError, extensionsQuery.error]);

  // Per-file reset. `path` is controlled by the host, so this replaces the old
  // imperative openFile(): whatever opens a file just changes the prop.
  useEffect(() => {
    if (!path) return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- setters are stable; re-running on displayName would reset mid-view
  }, [path]);
  ...
  // View shortcuts are scoped to this container, not `window`. Two instances can
  // be mounted at once (the standalone page stays mounted-but-hidden while the
  // project overlay is open), and a hidden subtree can never hold focus — so
  // scoping makes cross-talk structurally impossible instead of relying on a
  // coordination flag. Focus is claimed on mount, on file change, and on click.
  const rootRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (hasFile) rootRef.current?.focus({ preventScroll: true });
  }, [hasFile, path]);

  const onContainerKeyDown = (e: React.KeyboardEvent) => {
    const tag = (e.target as HTMLElement).tagName;
    if (tag === "INPUT" || tag === "TEXTAREA" || tag === "SELECT") return;
    if (e.key === "f" && hasFile) imageRef.current?.fitToWindow();
    else if (e.key === "1" && hasFile) imageRef.current?.oneToOne();
  };
  ...
  const parseHeaderCoord = (isRa: boolean, ...keys: string[]) =>
    parseHeaderCoordShared(headerCards, isRa, ...keys);
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", flexDirection: "column", flexGrow: 1, minWidth: 0, height: "100%" }}>
        ...
        {/* Empty state — supplied by the host (the page offers Browse). */}
        {!hasFile && emptyState}
      </Box>
      ...
      <PlateSolveDialog
        ...
        headerFocalLength={headerNumber(headerCards, "FOCALLEN")}
        headerPixelSize={headerNumber(headerCards, "XPIXSZ")}
        headerBinning={headerNumber(headerCards, "XBINNING")}
        onSolved={(res) => {
